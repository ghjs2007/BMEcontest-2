...
refactor's own process artifacts, not consumers: the deletion ledger itself and
the delivery specification (``docs/BMEcontest-2 ... .md``) plus the historical
plan files under ``docs/superpowers/``, which name cleanup categories by design.
...
def _scan_texts(scopes: tuple[str, ...]) -> list[tuple[str, str]]:
    ...
    for path in files:
        ...
        if relative in _EXCLUDED_FILES or relative.startswith(_EXCLUDED_DIR_PREFIXES):
            continue
