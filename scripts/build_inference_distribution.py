...
_GENERATED_FILES = frozenset({"predict.py", "manifest.json", "feature_schema.json", "requirements.txt", "README.md"})
...
def runtime_source_closure(repository_root: Path) -> tuple[Path, ...]:
    """Compute the complete import-time canonical source closure for Predictor."""
    root = Path(repository_root).resolve()
    pending = [path for module in _RUNTIME_ROOT_MODULES if (path := _module_path(root, module))]
    if len(pending) != len(_RUNTIME_ROOT_MODULES):
...
def copy_canonical_runtime_source(repository_root: Path, destination: Path) -> tuple[str, ...]:
    """Vendor exactly Predictor's parsed canonical closure under ``event_stack``."""
    root = Path(repository_root).resolve()
    destination.mkdir(parents=True, exist_ok=False)
    copied: list[str] = []
    for source in runtime_source_closure(root):
...
def verify_distribution_manifest(package: Path) -> None:
    ...
    for name in _GENERATED_FILES | {"event_stack", "models"}:
    ...
    if set(actual) - _GENERATED_FILES - {"feature_schema.json", "requirements.txt", "predict.py", "README.md"} - set(source_files) - set(model_files):
...
def build_inference_distribution(*, repository_root: Path, bundle_path: Path, destination: Path) -> Path:
    ...
    if bundle != expected:
        raise ValueError("inference distribution must use the current promoted deployment bundle")
    ...
    try:
        staging.mkdir()
        copy_canonical_runtime_source(root, staging / "event_stack")
        shutil.copytree(bundle, staging / "models", symlinks=False)
        (staging / "predict.py").write_text(_PREDICT_ENTRYPOINT, encoding="utf-8", newline="\n")
        ...
        probe = subprocess.run(
            [sys.executable, "-I", "predict.py", "--help"], cwd=staging,
